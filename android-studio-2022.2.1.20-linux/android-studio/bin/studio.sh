...
if [ -z "$JRE" ] && [ -s "${CONFIG_HOME}/Google/AndroidStudio2022.2/studio.jdk" ]; then
  USER_JRE=$(cat "${CONFIG_HOME}/Google/AndroidStudio2022.2/studio.jdk")
...
if [ -n "$STUDIO_VM_OPTIONS" ] && [ -r "$STUDIO_VM_OPTIONS" ]; then
  ...
  elif [ -r "${CONFIG_HOME}/Google/AndroidStudio2022.2/studio64.vmoptions" ]; then
    USER_VM_OPTIONS_FILE="${CONFIG_HOME}/Google/AndroidStudio2022.2/studio64.vmoptions"
  fi
fi
