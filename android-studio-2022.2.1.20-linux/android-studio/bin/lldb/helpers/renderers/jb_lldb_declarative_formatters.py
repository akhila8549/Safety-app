...
class AbstractStream(Stream):
    def output_object(self, val_non_synth: lldb.SBValue):
        ...
        vis_descriptor = get_matched_visualizers(val_non_synth.GetType(), format_spec)
        self.level += 1
        ...
        try:
            if vis_descriptor is not None:
                ...
                self._output_object_fallback(val_non_synth)
        finally:
            set_recursion_level(prev_level)
            self.level -= 1

    def _output_object_fallback(self, val_non_synth):
        # fallback to default summary
        summary_value = val_non_synth.GetSummary() or val_non_synth.GetValue() or ''
        self.output(summary_value)


class FormattedStream(AbstractStream):
...
class SimpleStream(AbstractStream):
    def output_string(self, text):
        self.output(text)
...
def declarative_summary(val: lldb.SBValue, internal_dict):
    try:
        ...
        stream_type = is_enabled_formatting() and FormattedStream or SimpleStream
        stream: AbstractStream = stream_type(is64bit, get_recursion_level())
...
class DeclarativeSynthProvider(object):
    ...
    def ensure_initialized(self):
        ...
        try:
            ...
            vis_descriptor = get_matched_visualizers(self.val_non_synth.GetType(), format_spec)
            if vis_descriptor:
...
g_type_to_visualizer_cache = {}
g_type_to_raw_view_visualizer_cache = {}


def get_matched_visualizers(value_type: lldb.SBType, format_spec: int) -> AbstractVisDescriptor:
    if format_spec & eFormatRawView:
        cache = g_type_to_raw_view_visualizer_cache
        use_natvis = False
    else:
        cache = g_type_to_visualizer_cache
        use_natvis = True
