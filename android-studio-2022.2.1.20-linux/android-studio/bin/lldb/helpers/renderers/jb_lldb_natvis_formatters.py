    ...
    def output_summary(self, value_non_synth: lldb.SBValue, stream: Stream):
        for name_viz_pair in self.viz_candidates:
            ...
            try:
                ...
                # TODO: implement output_summary() for natvis
                # try to choose candidate from ordered display string expressions
                summary_str = _find_first_good_node(viz.summaries, _process_summary_node, value_non_synth, matches)
                if summary_str is not None:
                    stream.output(summary_str)
                    return
...
def _evaluate_interpolated_string(interp_string: TypeVizInterpolatedString, ctx_val, wildcards=None, context=None,
    ...
    for expr in interp_string.expr_list:
        substring = _eval_display_string_expression(ctx_val, expr, wildcards, context, disable_formatting) or ''
...
def _process_summary_node(summary: TypeVizSummary, ctx_val: lldb.SBValue, wildcards):
    # ctx_val is NonSynthetic
    if summary.condition:
        if not _process_node_condition(summary.condition, ctx_val, wildcards):
            return None

    return _evaluate_interpolated_string(summary.value, ctx_val, wildcards)
...
def _resolve_wildcards_in_interpolated_string(interp_string: TypeVizInterpolatedString, wildcards):
    ...
    for expr in interp_string.expr_list:
        ...
        expr_list.append(TypeVizExpression(text, array_size, format_spec, view_spec))

    return TypeVizInterpolatedString(interp_string.fmt, expr_list)
...
def _eval_display_string_expression(ctx, expr, wildcards, context=None, disable_formatting=False):
    rec_level = get_recursion_level()
    if rec_level >= g_max_recursion_level:
        return ''
    ...
    is_formatting_enabled = is_enabled_formatting()
    if disable_formatting:
        enable_disable_formatting(False)
    try:
        display_string = result.GetSummary()
        if not display_string:
            result_non_synth = result.GetNonSyntheticValue()
            display_string = result_non_synth.GetValue()
    finally:
        enable_disable_formatting(is_formatting_enabled)
...
@optional_node_processor
def _node_processor_display_value(node: Union[TypeVizItemConditionalNodeMixin,
                                              TypeVizItemFormattedExpressionNodeMixin,
                                              Optional[TypeVizItemNamedNodeMixin]],
...
class LinkedListCustomNameNodesProvider(NodesProvider):
    def __init__(self, size, head_pointer, next_expression, custom_value_name, wildcards):
        ...
        while it and idx < max_size:
            ...
            name = _evaluate_interpolated_string(custom_value_name, cur_val, wildcards, disable_formatting=True)
            names.append(name)
            name2index[name] = idx
...
def _node_processor_tree_items(tree_node, ctx_val, wildcards):
    ...
    size = _find_first_good_node(tree_node.size_nodes, _node_processor_size, ctx_val, wildcards)
    # size can be None
...
class CustomListItemsItemInstruction(CustomListItemsInstruction):
    ...
    def execute(self, ctx_val: lldb.SBValue, context, items_collector: List[lldb.SBValue]):
        if self.evaluate_condition(ctx_val, context):
            if self.name:
                name = _evaluate_interpolated_string(self.name, ctx_val, context=context, disable_formatting=True)
            else:
